...
export function verifyJWT(token: string) {
    try {
      const decoded = jwt.verify(token, publicKey) as JwtPayload; // Explicitly cast the decoded object to JwtPayload
      return { payload: decoded, expired: false };
    } catch (error) {
      return { payload: null, expired: (error as VerifyErrors)?.message.includes("jwt expired") };
    }
  }
